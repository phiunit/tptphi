...
- L3 critic done → r3-critic-L3.md. FIXED 68b3943 with two shared gate corrections: printed model sentences no longer become typeable fields (L3 lost 12 spurious ones, news-desk-frames 9), and a slide may mark an assessed item `verbatim` to skip the character cap while keeping the geometric overflow gate (L2's exit item restored word for word). Title slides no longer print the chip row twice.
- L4 critic done → r3-critic-L4.md: every surface REJECT, 26 findings, same root cause as L5. FIXED 6c3c7fc (audit green; ISTE 1.6.b → 1.1.c; name field 153pt; new block split 6·6·10·18·5).
- L6 critic done → r3-critic-L6.md: cover and listing SHIP, four surfaces REJECT, 21 findings. Root causes: a pass-direction that was never physically specified (it broke the product's only filled answer key), and a write-in idiom that drew a 68px empty box with one rule at the bottom. FIXED 54ecf23 (audit green; rotation traced against the filled key; worksheet 2 → 3 pages with every count updated).
- All six lessons have been through Round 3 and all six are fixed and pushed (L1+L5 77234a3 · L2 69181c2 · L3 68b3943 · L4 6c3c7fc · L6 54ecf23). Cross-product SWEEP builder running.
- A bundle-freshness gate now blocks the one defect nothing else could see: a bundle whose zip packs a child's superseded file. Proven by deliberately changing a child and watching validate name it (rule 19 in ANTI_SLOP).
- Round 3's five recurring defects are now rules 14–18 in docs/ANTI_SLOP.md and a paragraph in docs/GAUNTLET.md (cb59afb), so future critics and builders inherit them.
- News Desk had its first critic → docs/gauntlet/2026-09-11/critic-news-desk.md. Deck SHIP, everything else REJECT, 22 findings. Central one: "works on any story" was asserted, not engineered — Frames 4, 5, 6 and 8 need the story to supply something it may not have, and only Frame 10 carried a fallback. Also a math standard that does not cover percent change (6.RP.A.3c → 7.RP.A.3) and a coin budget that never binds. FIXED 31536d7 (both audits green; Frame 10 repriced 2·3·5·7·9 so five of ten pairs exceed the 10-coin budget; fallbacks on Frames 4, 5, 6, 8; 7.RP.A.3; FACTS.md 21 lines, all PENDING).
