...
## NEWS DESK FIXES — report docs/gauntlet/2026-09-11/critic-news-desk.md, all 22 accepted
Files: products/news-desk-frames/src/{frames.html, lesson-plan.html, teacher-guide.html, worked-examples.html, slides.yaml, product.cover.html}, products/news-desk-frames/{product.yaml, FACTS.md}, and products/news-desk-frame-free/**. Both products stay `status: draft` in yaml and catalog. Use slug-scoped audits only.
- **Frame 10 pricing**: reprice the five protections 2 · 3 · 5 · 7 · 9 coins with the two most valuable at 7 and 9, budget stays 10. Exactly half the pairs fit, so the budget binds and "you can't protect everything" is actually tested. Recompute the teacher guide's worked rating so its numbers match.
- **Fallback clauses**: Frames 4, 5, 6 and 8 each gain one clause letting the student substitute from their own life when the story does not supply what the frame needs — modeled on Frame 10's existing "from the story, or your own phone, game saves, bike". Write each in that frame's own voice. Frame 5: "or, if the story names fewer, the plans for one app you already use." Frame 6: an old and a new price the student knows. Frame 4: a free thing they use. Frame 8: "if the story doesn't say who paid, write 'not stated' and say what that tells you."
- **Frame 7** `:412`: the "Where I draw the line" line becomes a `.frame` model sentence ("Need a start? Copying a ______ is fine when ______. It crosses the line when ______.") above its two ruled lines, so it prints as an example and stops generating clipped fields.
- **`:492`**: add `data-nofill` to the list item (or the whole `.panel`) so the underscore run inside the printed method stops becoming a field. After render, confirm widget p9_217 is gone and the pack's field count drops by one more.
- **Frame 3**: cut the rules grid from four rules to three.
- **Lesson plan**: Support line → "Start with Frames 1, 7 or 9 (fewest steps)"; the "Need a start?" list is correct only once Frame 7 is fixed, so verify it. Bell-ringer list gains Frame 7. Name Frames 3 and 6 in the Frame block as the two that take a longer solo split (solo 16 / pair-check 4 instead of 12 / 8).
- **Math standard**: CCSS Math 6.RP.A.3c → **7.RP.A.3**, text character-identical to curriculum/standards-registry.yaml, in product.yaml, the lesson-plan chips, the teacher guide's standards map, the description's standards line, and the deck's Frame 6 notes. 6.RP.A.3c covers percent of a quantity, not percent increase and decrease.
- **Worked examples**: `:143` "fans asked the obvious question" → the frame's own question, asserting nothing about anyone's reaction ("Free sites still cost money to run. So if it stays free, who pays?"). `:104` "The company will have its turn to answer in court." → generalize to how lawsuits work, not this case. Nothing in this file may assert a reaction, a motive or a future event without a FACTS.md line. Apply one anonymization convention across both the story panel and the filled frame, and state it in the sub-line. Update FACTS.md so the ledger still matches statement for statement; every line stays PENDING.
- **Teacher guide** `:83`: "labelled" → "labeled".
- **Cover**: kicker → "Works On Any Story, Any Week" (it must carry the differentiator, not repeat the title). Chip → "AI Literacy · Media Literacy".
- **Listing**: drop or repair "fixed budget" depending on the repricing above (it is true once the budget binds); standards line reprinted with 7.RP.A.3; "8-week" → "eight-week"; the unit line → "It stands alone. The separate 6-lesson AI Literacy bundle saves 30%."
- **Free product**: student page stops selling — `frames.html:75` kicker → "Fact Desk · Is it true?" and `:117` footer → "Fact Desk — Frames". Split the 50-word funnel sentence in product.yaml into three sentences under 25 words each, and point the search phrase at words that are actually in the pack's title ("News Literacy Templates"). Cover chip "1 one-page frame" → "One page · One frame". Break the 27-word teacher-guide sentence at `:40`.
Verification: `npm run audit -- news-desk-frames` and `npm run audit -- news-desk-frame-free` GREEN; report the Frame 10 price table and which pairs now exceed the budget, the field count before and after the `data-nofill` fix, the FACTS.md line count, and screenshots of frames-p05.png, frames-p07.png, frames-p10.png, the pack cover at 200px, and the free product's frames page.
